...
public class BaseFrame extends JFrame {
    ...
    public BaseFrame(String title) {
        ...
        root = new JPanel(null); root.setOpaque(false); root.setBounds(0,0,980,1040); add(root);
        phone = UIUtil.softCard(); phone.setBounds(285, 18, 410, 960); root.add(phone);
        JLabel grain = new JLabel(""); grain.setOpaque(false); grain.setBounds(0,0,410,960); phone.add(grain);
        header = new JPanel(null); header.setOpaque(false); header.setBounds(24, 18, 360, 78); phone.add(header);
        content = new JPanel(null); content.setOpaque(false); content.setBounds(24, 104, 360, 770); phone.add(content);
        nav = new JPanel(null); nav.setOpaque(false); nav.setBounds(24, 888, 360, 58); phone.add(nav);
    }
